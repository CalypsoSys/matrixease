...
function updateChords(keys, matrix) {
    ...
    groupG.select("title")
        .text(function (d, i) {
            if (i == 0) {
                return keys[i].name + " is key"
            } else {
                return numberWithCommas(d.value) + " rows in " + keys[i].name;
            }
        });
    ...
    chordPaths.select("title")
        .text(function (d) {
            if (keys[d.target.index].name !== keys[d.source.index].name) {
                return [numberWithCommas(d.source.value),
                    " row from ",
                    keys[d.source.index].name,
                    " in ",
                    keys[d.target.index].name/*,
                    "\n",
                ...
                return numberWithCommas(d.source.value) + "No Correlation " + keys[d.source.index].name;
            }
        });
    ...
    chordPaths.transition()
        ...
        //.attr("opacity", 0.5) //optional, just to observe the transition
        .style("fill", function (d) {
            return keys[d.source.index].color;
        })
