...
namespace MatrixEase.Web
{
    ...
    public class GoogleController : ProcessController
    {
        ...
        public GoogleController(ILogger<GoogleController> logger, IBackgroundTaskQueue queue, IOptions<AppSettings> options) : base(queue)
        {
            _logger = logger;
            _options = options;
        }
        ...
        public RedirectResult Get(string matrixease_id)
        {
            ...
            catch (Exception excp)
            {
                ...
                ClearCookies();
                return Redirect(BuildFrontendRedirectUrl("?auth=denied"));
            }
        }
        ...
        private string BuildFrontendRedirectUrl(string suffix = "")
        {
            string frontendBaseUrl = _options.Value.FrontendBaseUrl ?? "/";
            ...
            return frontendBaseUrl + "/" + suffix.TrimStart('/');
        }
    }
}
